...
def main():
    ...
    # model = mnist_net()
    # checkpoint = torch.load('pgd_attack.pt')
    model = Net().cuda()
    # model.load_state_dict(torch.load('pgd_attack.pt'))
    # model.load_state_dict(torch.load('fgsm_attack.pt')) 
    torch.save(model.state_dict(), "2023-01-19_97.73999786376953.pth")
    model.eval()
