...
from NEU_train import *
from NEU_model import *
from NEU_test import * 
...
class FedAvg:
    ...
    def client_update(self, index, device,non_iid,eps):  # update nn
        ...
        for k in index:
            ...
            self.nns[k] = train_NEU(self.nns[k],non_iid,lr,optimizer_name= optimizer,client_epoches = E, trained_clinet_number=k,total_clients= total_clients)
            print(self.nns[k])
        '''
        # if self.dataset == "uv":
        #     for k in index:
        ...
        #         else:
        #             print("The client_{} start to train(clean)".format(k))
        #             self.nns[k] = train_uv(self.nns[k],non_iid,lr,optimizer_name= optimizer,client_epoches = E, trained_clinet_number=k,total_clients= total_clients)
        
        # elif self.dataset == "NEU":
        #     print("訓練六分類問題")
        #     for k in index:
        #         print("The client_{} start to train(clean)".format(k))
        #         self.nns[k] = train_NEU(self.nns[k],non_iid,lr,optimizer_name= optimizer,client_epoches = E, trained_clinet_number=k,total_clients= total_clients)
    ...
    def aggregation(self, index): #question
        ...
        #把loss值寫出來另外保存
        f = open("./result/every_round_r2.txt", "a") #更改路徑!!!
    ...
    def every_round_test(self,agg_model):
        ...
        every_round_r2 = test_NUE(every_round_model)
        print("===")
        print(every_round_r2)
